...
fun InstallDialog(
    ...
    val systemModeDialog = rememberConfirmDialog()
    val systemModeMsg = stringResource(
        if (installUiState.method == InstallViewModel.Method.SYSTEM_MODE_RECOVERY)
            CoreR.string.system_mode_recovery_msg else CoreR.string.system_mode_warning
    )

    LaunchedEffect(installUiState.showSystemModeWarning) {
        if (installUiState.showSystemModeWarning) {
            val result = systemModeDialog.awaitConfirm(title = secondSlotTitle, content = systemModeMsg)
            installVm.onSystemModeWarningConsumed()
            ...
            Scaffold(
                ...
                Column(
                    ...
                    Card(
                        ...
                        if (installVm.isRooted && !Info.isSystemMode) {
                            SettingsArrow(
                        ...
                        if (installVm.allowSystemMode) {
                            SettingsArrow(
                                title = stringResource(CoreR.string.direct_install_system_mode),
                                onClick = {
                                    onDismiss()
                                    installVm.selectMethod(InstallViewModel.Method.SYSTEM_MODE)
                                },
                            )
                            if (Info.isSystemMode) {
                                SettingsArrow(
                                    title = stringResource(CoreR.string.system_mode_recovery),
                                    onClick = {
                                        onDismiss()
                                        installVm.selectMethod(InstallViewModel.Method.SYSTEM_MODE_RECOVERY)
                                    },
                                )
                            }
                        }
