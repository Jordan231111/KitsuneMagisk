...
> [GitHub #27](https://github.com/Jordan231111/KitsuneMagisk/pull/27) at `c02320a92`. The repository's
> default branch remains `kitsune` (`origin/HEAD` points to `origin/kitsune`); there is intentionally
> no separate branch named `main`. `next-system` was then cut directly from official v30.7 and PR6
> completed at `eab2968c9`. Work currently stays on `next-system`; reading this roadmap from
> `kitsune` does not authorize merging or committing development changes there.
...
| Expanded `MODDIR/root` partition API (`odm`, `*_dlkm`, `oem`, `apex`, prism/optics, Oppo/OnePlus `my_*`) | Shipped Kitsune module API absent from v30.7's standard partition set | PR14 must port a capability-versioned safe subset or retire individual partitions with module/device evidence; an unassigned loss is forbidden |
...
| Pre-init storage selection that avoids `/persist` | Historical Kitsune removed `/persist`; official v30.7 retains it as a dangerous last resort | PR14 defines the policy and PR15 validates boot/recovery; never change storage choice without an explicit migration/safety result |
...
- [x] Runtime use of exact install-manifest ownership across install/recovery/uninstall, including
  host fault injection and repeated-death recovery. Live writable-target qualification remains a
  separate unchecked lifecycle gate above.
...
# Mandatory historical feature closure inventory
...
status. PR16 must close all three classes.

| Stable family | Historically shipped behavior and affected use case | Historical evidence / upstream relationship | Owning PR and mandatory closure |
|---|---|---|---|
...
| HID-02 | Inverse SuList semantics: only selected apps receive SU/module visibility, with separate list/settings/CLI behavior | Historical `sulist` table and SuList UI/commands; absent from official | PR12 must preserve or explicitly retire selection, inverse policy and migration separately; PR13 UI; PR15 multiuser/runtime evidence |
| HID-03 | Zygote/app monitoring through ptrace, USAP handling, isolated processes, secondary users/work profiles, package rescans and zygote restarts | Historical deny monitor/process tracker across fork commits; maintained upstream DenyList overlaps only partly | PR12 owns bounded process lifecycle and performance tests; PR15 covers real multiuser/USAP/isolated processes |
| HID-04 | Per-app mount-namespace cleanup hiding Magisk mounts, module mounts/files and injected paths | Historical namespace/mount hiding and shared-tmpfs interaction | PR12 owns measured visibility contract; PR14 supplies module/mount semantics; PR15 proves representative apps and recovery |
| HID-05 | Dynamic `/system/bin` MagiskSU/applet exposure used with Hide/SuList | `fb1fd3d6`; upstream injection design changed | PR12 retains only with namespace/module/ordinary-app/uninstall evidence, otherwise `retired-with-evidence` and migration |
| HID-06 | Randomized daemon socket, `/dev` daemon/FIFO names and related IPC concealment | `4a6e433b` and historical randomized runtime endpoints; upstream protocol evolved | PR12 threat-models protocol, restart and multi-instance behavior, then ports or retires; PR16 verifies no stale compatibility claim |
| HID-07 | SELinux-disabled, permissive, Waydroid/container and unusual zygote environments | `c30ba784` and historical disabled-policy/container handling | PR12 defines core semantics; PR15 owns exact Waydroid/disabled/enforcing lanes or explicit unsupported results |
...
| ZYG-04 | Older-tag built-in Zygisk API v5, module loading, companion IPC, memfd loading, Riru exclusion and Deny/Hide integration | Shipped before `2ef8f002` removed built-in Zygisk | PR11 atomizes and tests inherited/ported behavior or retires each compatibility promise separately |
| ZYG-05 | Faster Zygisk revert/unmount path | `7072f9e9`; removed/superseded before the endpoint does not erase published behavior | PR11 measures correctness/performance and ports or retires it with evidence |
| ZYG-06 | Zygote restart-monitor reliability behavior | `be732edb`; independently observable across daemon/zygote restarts | PR11 owns provider/core lifecycle tests and an explicit disposition |
| MOD-01 | Global and per-module early mount API and ordering | `3dcfaf9f`, historical early-mount paths; official pre-init/module engine is newer but not identical | PR14 versioned API, ownership, ordering, collision, timeout and safe-mode tests |
| MOD-02 | Module `initrc.d`/init-RC injection | `9e1e9ad7` and historical module install/boot integration | PR14 ports a constrained API or retires it with module migration; PR15 tests retained init subcontexts |
| MOD-03 | `MODDIR/root` module layout and expanded partitions: standard targets plus retained `odm`, `*_dlkm`, `oem`, `apex`, prism/optics and `my_*` OEM paths | `5d88f4c1` and module installer/magic-mount paths; upstream covers a changing standard subset | PR14 publishes exact supported/retired names and collision semantics; PR15 validates representative OEM layouts |
| MOD-04 | Magic-mount topology, shared tmpfs, binary injection and systemless-hosts behavior | `e2e83318`, `4763e442`, `b8d3735f`, `2b20184c`; v30.7 Rust engine is substantially redesigned | PR8 behavior comparison; PR14 closes visibility, ordering, mount-ID, host-file and uninstall differences with tests |
| MOD-05 | Systemless deletion, whiteouts and `.replace` directory semantics | `0933b53b` plus historical mount implementation; some behavior is upstream under a new engine | PR14 inherits-and-tests exact behavior or records per-mechanism migration/retirement |
| MOD-06 | Module `sepolicy.rule` live refresh and pre-init synchronization | `87d5c29d`; upstream consolidates rules but not the exact shipped refresh contract | PR14 owns install/update/disable/remove/reboot/interruption and enforcing/permissive/disabled tests |
| MOD-07 | Module action and boot-stage scripts including `action.sh`, `boot-completed.sh`, post-fs-data/service behavior | `0fc898ca`, `63ddd078`; action is upstream, historical boot-completed contract is not | PR14 defines bounded ordering, logging, timeout, disable/remove and recovery or explicitly retires each extra stage |
| MOD-08 | Ordinary module install, enable, disable, update, removal, rollback, interrupted update, safe mode and remove-modules recovery | Historical module manager/daemon plus ordinary Magisk behavior | PR14 owns full lifecycle; PR15 representative targets; one successful marker mount is insufficient |
...
| SU-04 | Daemon/client authorization, `restorecon`, and SU execution when SELinux is disabled | `9fe558af`; distinct from Hide's zygote detection in that environment | PR12 owns native/core semantics; PR13 owns request-policy UI; PR15 qualifies exact disabled-policy targets |
| APP-01 | Hidden/randomized manager package creation, launch, upgrade, restore and signer-failure recovery | Inherited hidden manager plus `365ad294` package randomization | PR13 threat model and lifecycle; PR15 every advertised target class; retain randomization only with recovery evidence |
| APP-02 | Manager/stub package-signature enforcement and historical global bypass | `c12fca79` disabled enforcement; PR #26 restored release enforcement | PR8 records bypass as a retired security regression; PR10/PR13 own verified transitions; PR16 rejects wrong signer and historical public test identities |
| APP-03 | Stub/full-manager download, update, replacement, API 34 lifecycle fixes and exact-byte integrity | Historical stub verification/download/API 34 fixes; v30.7 overlaps but project trust root does not exist | PR10 owns project metadata, atomic download, signer/package/downgrade checks; PR16 verifies published bytes end to end |
| APP-04 | FakeGMS/ProviderInstaller compatibility and validation | `ea72fbd4`; maintained upstream behavior requires semantic comparison | PR8 identifies affected apps; PR13 inherits-and-tests or ports the minimal delta; otherwise explicit retirement |
| APP-05 | Product UI/UX shipped to users: Hide/SuList/System Mode surfaces, module-action focus/close, lock-screen orientation, diagnostics, accessibility and translations | Historical app/resources/locales and effective UX fixes | PR8 enumerates atomic deltas; PR13 ports or retires with user impact and safe locale fallback |
| APP-06 | Historical non-obfuscated manager/stub and globally disabled optimizer behavior | `0128bb18`; official enables normal optimization/obfuscation | PR8 separates debuggability from unsafe optimization disablement; PR10/PR16 retain source availability and symbol mapping while restoring safe release optimization |
...
| BOOT-02 | 16 KiB kernel page and ELF/ZIP alignment support, including the history of enable/revert/fix commits | Multiple historical commits; maintained official has evolved support | PR8 atomic history; PR15 actual 16 KiB runtime; PR16 artifact/parser gates on exact bytes |
| BOOT-03 | Boot/recovery tar archives larger than 8 GiB and large-entry extraction/creation | Historical magiskboot/app tar fixes; upstream overlap must be verified | PR8 exact delta; PR15 large corpus and representative Samsung-style package; PR16 regression gate |
| BOOT-04 | `magiskboot` split/decompression `-n` and related command compatibility | Historical tooling changes used by scripts/modules; upstream CLI may differ | PR8 identifies callers; PR15 corpus/script compatibility or actionable retirement |
...
| ORD-04 | Public MagiskBoot, `resetprop`, `magiskpolicy`, Magisk multicall applets and bundled BusyBox command compatibility | Historical README/tool/module contracts; System Mode and app smoke do not cover external callers | PR8 records exact callers/deltas; PR15 runs CLI/corpus/module compatibility; PR16 publishes retained/retired command contracts |

## Mandatory PR8 atomic seed

The family rows above are navigation, not acceptable final ledger granularity. The following is the
minimum atomic seed derived from the complete endpoint delta and frozen tag universe. PR8 may split
or add rows when history shows different commits, tags, users, decisions, or evidence, but it may
not merge, delete, or silently rename these decision units. PR16's checker must require a terminal
state for every stable ID below and every additional generated row.

### System Mode

| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `SYS-001` | Direct-System core install on a writable system layout | PR7 |
| `SYS-002` | Manager entry, eligibility, warning, results, and recovery UI | PR7, then PR13 |
| `SYS-003` | Persistent payload, ABI binaries, config, init triggers, tmpfs/applet bootstrap | PR7 |
| `SYS-004` | Live/offline SELinux policy selection, backup, patch, load, and restoration | PR7 |
| `SYS-005` | Upgrade/reinstall and migration from legacy payload, RC, config, sidecar, and addon layouts | PR7 |
| `SYS-006` | Exact System Mode uninstall and restoration of init/policy/runtime state | PR7 |
| `SYS-007` | Explicit recovery/ZIP `SYSTEMMODE` entry | PR15 |
| `SYS-008` | Implicit privileged `systemmagisk` filename trigger | PR7 retirement, PR15 migration |
| `SYS-009` | Custom-ROM OTA/`addon.d` survival | PR15 |
| `SYS-010` | Historical API 23–24 System Mode availability/claim | PR7 retirement or PR15 qualification |

### MagiskHide and SuList

| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `HID-001` | Classic MagiskHide enable/disable/status independent of SuList and built-in Zygisk | PR12 |
| `HID-002` | Per-package/process add/remove/list persistence and CLI | PR12 |
| `HID-003` | Manager list/filter/system-app/expansion/per-process selection UI | PR13 |
| `HID-004` | `magiskhide exec` isolated mount-namespace command | PR12 |
| `HID-005` | SuList enable/disable, Hide dependency, reboot-only transition, and live-disable refusal | PR12 |
| `HID-006` | SuList settings/list UI and reboot-mismatch messaging | PR13 |
| `HID-007` | Inverse root/module visibility plus automatic manager/SystemUI/Settings enrollment | PR12 |
| `HID-008` | Ptrace zygote/fork monitor, USAP, preinitialized processes, app zygotes, and restarts | PR12 |
| `HID-009` | Isolated/prefix matching, shared UID, secondary users/work profiles, and package rescans | PR12 |
| `HID-010` | Per-process cleanup of Magisk/module/early-mount/injected visibility | PR12 with PR14 |
| `HID-011` | Daemon's own internal namespace visibility after `ba6ed414` | PR12 |
| `HID-012` | Dynamic `/system/bin` MagiskSU/applet exposure and zygote-restart unmount | PR12 |
...
| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `ZYG-001` | Older-tag built-in API v5, module loading, companion IPC, memfd loading, Riru exclusion, and Hide integration | PR11 |
| `ZYG-002` | Ptrace-init injection architecture | PR11 |
| `ZYG-003` | GrapheneOS Android 14/QPR2 specialization compatibility | PR11 |
| `ZYG-004` | Futile Hide handling for Zygisk module libraries | PR11 |
...
| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `MOD-001` | Global early-mount files and ordering | PR14 |
| `MOD-002` | Per-module `early-mount` storage, install, disable, and removal | PR14 |
| `MOD-003` | `initrc.d` init-RC injection | PR14 |
| `MOD-004` | `MODDIR/root` standard and expanded partition API | PR14 |
| `MOD-005` | Conditional Magisk binary injection when PATH already exposes tools | PR14 |
| `MOD-006` | Systemless-hosts read-only and visibility behavior | PR14 |
| `MOD-007` | Mirrorless magic-mount topology and visibility | PR14 |
| `MOD-008` | Shared tmpfs mount propagation to later namespaces | PR14 |
| `MOD-009` | `REMOVE` char-device whiteout deletion | PR14 |
| `MOD-010` | `.replace` directory semantics | PR14 |
| `MOD-011` | Live `sepolicy.rule` application and pre-init refresh/synchronization | PR14 |
| `MOD-012` | Empty-line-safe module policy parser | PR14 |
| `MOD-013` | `action.sh` discovery and execution backend | PR14 |
| `MOD-014` | Module Action manager surface | PR13 |
| `MOD-015` | Per-module `boot-completed.sh` stage | PR14 |
...
`my_engineering`, `my_heytap`, `my_manifest`, `my_preload`, `my_product`, `my_region`, `my_stock`,
`my_version`, `my_company`, `my_carrier`, and `my_bigball`. A generic “OEM partitions” row is not
closure evidence.
...
| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `SU-001` | Disabled, system-credential, and manager biometric authentication modes | PR13 |
| `SU-002` | Prompt/allow/deny/revoke/timeout, namespace, shared UID, multiuser, and no-manager lifecycle | PR13 |
| `SU-003` | Deterministic manager policy-row retrieval (`LIMIT 1`) | PR13 |
| `SU-004` | Daemon/client authorization, `restorecon`, and SU execution with SELinux disabled | PR12 with PR13 |

### Manager, identity, update, and distribution

| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `ID-001` | Kitsune package/application identity and upgrade path from `io.github.huskydg.magisk` | PR9 |
| `APP-001` | Kitsune Mask name, labels, logos/colors, attribution, and unofficial-build warning | PR13 |
| `APP-002` | Hidden-manager create/launch/upgrade/restore plus `com.*` random package generation | PR13 |
| `APP-003` | Manager/stub signature enforcement and retirement of the global bypass | PR10 with PR13 |
| `APP-004` | API 34 download engine, user-initiated jobs, foreground-service behavior, and suspendable I/O | PR10 |
| `APP-005` | Stub API 34 loading, expected-version self-repair, and manager-driven stub upgrade | PR10 |
| `APP-006` | HuskyDG/1q23 endpoints, channels, full-manager/stub URL migration, and failure behavior | PR10 |
| `APP-007` | FakeGMS ProviderInstaller restricted to system GMS | PR13 |
| `APP-008` | Inactive-slot OTA prompt requiring the System Updates Restart action | PR13 with PR15 |
| `APP-009` | Module-action close/focus/auto-close behavior | PR13 |
...
| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `BOOT-001` | P-521 boot-image signing and verification | PR15 |
| `BOOT-002` | 16 KiB binary/ZIP/runtime support, including disable/re-enable history | PR15 |
| `BOOT-003` | Tar input/output with entries larger than 8 GiB | PR15 |
| `BOOT-004` | `magiskboot split -n` compressed-kernel preservation | PR15 |
| `BOOT-005` | 2SI transition avoiding `/init` hexpatch when possible | PR15 |
...
| Atomic ID | Independently closable behavior | Terminal owner |
|---|---|---|
| `ORD-001` | Select-and-Patch for `boot`, `init_boot`, `vendor_boot`, recovery, and supported archives | PR15 |
| `ORD-002` | Ordinary Direct Install | PR15 |
| `ORD-003` | Inactive-slot installation and OTA completion | PR15 |
| `ORD-004` | Ordinary recovery ZIP install/uninstall and stock restoration | PR15 |
| `ORD-005` | Emulator live setup independent of persistent System Mode | PR15 |
| `ORD-006` | MagiskBoot, `resetprop`, `magiskpolicy`, multicall applets, and bundled BusyBox compatibility | PR15 |

MagiskSU, modules, safe mode, hidden manager, DenyList, and Zygisk ordinary parity are represented by
their dedicated rows rather than duplicated here. Dependency-only bumps, CI enable/disable churn,
AGP/Gradle/ONDK updates, formatting/logging refactors, LTO workarounds, dead-submodule mirror changes,
and documentation-only changes are excluded only when PR8's generated commit/path audit proves they
have no material user-visible behavior. A commit message alone is insufficient to exclude a row.

PR8's generated ledger must add the exact commit(s), path(s), first/last published tag, affected-user
description, upstream comparison, chosen decision, owner PR, evidence URI and migration text for each
atomic row. It must also record commits that only fixed or reverted another listed feature so the
final disposition reflects the behavior users actually received. Security regressions are retained
as closure rows and deliberately retired; they are never reintroduced in the name of fidelity.
...
**Implementation status: complete on `next-system` at `eab2968c9`.** The branch was cut directly
from official v30.7 (`e8a58776f`) after it remained the latest stable at the 2026-08-01 branch cut.
The exact baseline, source delta, submodule gitlinks, build prerequisites, and artifact checks are
recorded in `docs/next-system-baseline.md` and its machine-readable companion. Re-resolve official
stable again before a later release candidate; do not rewrite this historical branch-cut record.
...
- Close every dependency blocker recorded by PR6 before promotion: use the public upstream CXX
  ...
  signing, and make both the exact lockfile contract and a fresh RustSec audit blocking CI gates.
...
- Bind authorization and the retained receipt to the concrete ADB serial, fingerprint, API, ABI
  list, source commit, and exact external-backup digest. `authorize` must re-hash a currently present
  backup file/tree before it stages anything; a stale report, same-fingerprint clone, missing backup,
  symlink/special-node backup, or dirty source fails closed.
- Replace self-attested qualification flags with a schema-validated machine record. Qualification
  must start with an absent backup destination, bind absolute non-symlink backup/restore/cold-boot
  wrappers by executable identity, write an unpredictable pre-backup anchor, create the backup,
  overwrite that anchor, and accept restore only when the original bytes and metadata return while
  post-backup probes disappear. It must also execute a temporary init helper through the production
  init directories/domains, observe three new boot IDs, preserve the backup bytes, reproduce the
  exact post-restore target contract, and bind a stable host-instance identity. A generic host
  command plus a changed boot ID is not a brand-level cold-boot claim: PR7 may qualify only the exact
  manually reviewed target wrapper, while PR15 owns vendor lifecycle adapters and wider claims.
  Bind the record, command identities, backup digest, and instance-identity digest into
  authorization, receipt, and manifest, then revalidate them at the one-shot handoff.
- Make the host handoff one shot: `authorize` performs `adb install -r` on the exact APK, immediately
  re-probes the pinned target, opens the manager Install screen, and exposes a random nonce only
  through an ephemeral `adb reverse` mapping. Re-hash backup/identity at nonce request, consume the
...
- Publish an independent versioned rescue payload and fixed init RC before the first boot-critical
  ...
  boot tree and terminal `UNINSTALLED` state are durable.
- Treat Android init `exec` exit status as non-propagating: wrap `--post-fs-data`, `--service`, and
  `--boot-complete`, publish a per-install success property only after a zero exit, and make boot
  verification require all three properties plus daemon-version and bounded root-request proof.
- PR7 Tier A may publish RC files only in `/system/etc/init` or `/system/etc/init/hw`. Vendor, ODM,
  product, and system-ext init subcontexts remain unqualified until PR15 proves their exact
  `vendor_init`/SELinux/property/exec behavior; writable storage alone is not import/execution proof.
- Retire the historical filename-substring `systemmagisk` ZIP trigger in PR7: the maintained
  in-guest path is an explicit debug-manager/host-CLI action bound to one doctor report. PR15 must
  either add a separately named, signed recovery/ZIP adapter with recovery-native target identity,
  mount, transaction, and rollback evidence, or close that historical route as
  `retired-with-evidence` with user-facing migration guidance. Never revive implicit filename-based
  privileged behavior.
- Do not claim custom-ROM OTA/`addon.d` survival in PR7. Remove an inherited legacy addon only inside
  the rollback transaction. PR15 owns the keep/port/retire decision and, if retained, must require
  reauthorization against the changed image instead of replaying stale policy or manifest data.
...
- Keep API 23–24 System Mode unavailable unless its dedicated persistent launcher is independently
  cold-boot qualified; continue testing ordinary Magisk on Android 6.
- Before completion, require clean debug/release all-ABI builds, lint/source/artifact gates, host
  transaction fault injection (including two consecutive hard deaths at every rescue/rollback
  boundary), Android 6/11/15 normal-route regressions, modern Android 16 and
  preview-17 coverage where runnable, and exact existing-MuMu install/reinstall/boot/uninstall
  evidence without deleting the user-owned instance.
- Permit exactly one registered cold-boot attempt for each new commit. If that boot never reaches
  manifest-bound verification, the next boot must recover the prior state before starting Magisk,
  preventing an experimental System Mode payload from becoming an unbounded boot loop.

**Exit:** at least one writable commercial emulator completes install, three cold boots, upgrade,
module/root smoke, uninstall, and byte/digest-verified external restore on the exact clean PR6-based
...
- Create `docs/port-ledger.md` with one row for every effective feature at pre-maintenance commit
  ...
  `ported-and-tested-in-PR#`, `deferred-to-PR#`, or `retired-with-evidence`; vague `triage`, an empty
  owner, or an unnumbered future task fails validation.
- Treat the ledger as a machine-checked promotion gate. A `deferred-to-PR#` row may name only
  PR9–PR15 and must transition by its owner PR to `inherited-and-tested`,
  `ported-and-tested-in-PR#`, or `retired-with-evidence`. PR16 rejects every unresolved, deferred,
  untested, or schema-invalid historical row regardless of P0/P1/P2 label. P3 is reserved for new
  post-release ideas, never a historically shipped behavior.
...
- Resolve dynamic `/system/bin` MagiskSU/applet exposure explicitly: retain it only if namespace and
  module compatibility tests prove the need, otherwise retire it with a migration/compatibility note.
- Threat-model and close the historical per-boot daemon-socket randomization: preserve a narrowly
  measured mechanism with protocol/restart/multi-instance tests, or retire it with evidence that
  maintained upstream socket ownership is safer and does not break a supported use case.

**Exit:** core behavior passes the required feature matrix without UI dependency.

## PR 13 — Product manager UI and SU authorization contract
...
- Preserve the historical SU authentication use case through a security-reviewed disabled/system-
  credential/manager-biometric contract, or explicitly retire a mode with migration rationale. The
  daemon/request backend is authoritative: test migration, cancellation, timeout, replay, process
  death, fallback, lockout, and manager/backend disagreement rather than treating this as a UI-only
  preference.
- Own the manager side of the ordinary MagiskSU lifecycle: prompt, allow, deny, revoke, timeout,
  shared UID, multiuser/work-profile, and manager-not-running behavior must agree with daemon policy.
- Own hidden-manager create, randomized-package disposition, upgrade, signer verification, launch,
  restore, and recovery UI. PR15 repeats the lifecycle on representative real targets. Random
  package behavior is retained only when its threat model and recovery cost justify it; otherwise
...
**Exit:** app, System Mode CLI/manifest, Hide CLI/DB, SU authorization backend, daemon, and provider
agree, including the retained authentication and hidden-manager lifecycles.

## PR 14 — Module compatibility closure and early-mount v2
...
- Define module `sepolicy.rule` live/pre-init refresh across install, update, disable, remove, reboot,
  permissive/disabled SELinux, and interrupted-copy cases.
...
- Qualify the complete ordinary module lifecycle—install, enable, disable, update, remove, rollback,
  ...
  only that one module can mount once.
- Define pre-init storage selection and migration, including the historical no-`/persist` safety
  choice, low-space handling, stale storage, encryption, and exact recovery.

**Exit:** every module-related historical ledger row is inherited and tested, ported and tested, or
retired with evidence; supported modules work on representative boot layouts and cannot trivially
strand a test device.
...
- Run the authoritative MagiskSU policy matrix on representative physical/emulator targets: prompt,
  allow, deny, revoke, timeout, shared UID, secondary user/work profile, namespace mode, daemon/app
  restart, and no-manager fallback.
- Repeat hidden-manager create, upgrade, launch, signer-failure recovery, and restore on every target
  class where that feature is advertised.
- Close the explicit recovery/ZIP System Mode and custom-ROM OTA/`addon.d` rows. If retained, run a
  real recovery install/uninstall and real ROM OTA/addon cycle with exact ownership, changed-image
  reauthorization, boot, root, rollback, and stock restoration. Otherwise publish the precise
  retirement and migration path.
- Reproduce and disposition the Vivo `do_mount_check` workaround on an exact owned target or a
  minimized kernel fixture; qualify Nox `/sbin`/PATH behavior and SELinux-disabled/Waydroid paths.
- Validate the selected pre-init storage policy and every retained expanded module partition on
  representative layouts; include negative `/persist`, low-space, wrong-device, and stale-copy cases.
...
- Require every PR1–PR15 exit condition and every applicable release checkbox to have linked
  evidence. PR16 is not allowed to turn an earlier missing feature into an unnamed future task.
- Require `docs/port-ledger.md` to contain no `deferred`, `unresolved`, or untested historical rows. Every
  historical feature at `8e854f378` and every tag-only shipped feature—including P2-labelled
  behavior and later-removed features—must be
  `inherited-and-tested`, `ported-and-tested-in-PR#`, or `retired-with-evidence`, with user impact,
  migration guidance, owner, and linked evidence. No `deferred-to-PR#` state survives PR16.

**Exit:** every release gate below is checked with linked evidence, every historical feature row is
closed, and no work required for a faithful production Kitsune release remains after PR16. P3 ideas
are optional post-release enhancements, not deferred historical compatibility work.
...
- [ ] Explicit recovery/ZIP System Mode and custom-ROM OTA/`addon.d` survival are either qualified
  end to end on exact targets or published as retired with actionable migration guidance; implicit
  filename triggers and stale-image replay are absent.
...
- [ ] Every advertised ABI—ARM64, ARM32, x86_64, x86, and any future RISC-V lane—has runtime boot,
  root, module, uninstall, and recovery evidence on an actual matching target; build-only ABIs are
...
- [ ] Early-mount plus `initrc.d`, ordering, collision, timeout, and safe-mode recovery pass.
- [ ] Expanded module partition API has an exact retained/retired target list and representative tests.
- [ ] Ordinary module install, enable, disable, update, remove, rollback, interrupted update, and
  safe-mode/remove-modules recovery pass.
- [ ] `boot-completed.sh`, module `sepolicy.rule` refresh, `action.sh`, whiteout/deletion, `.replace`,
  magic-mount/shared-tmpfs, and systemless-hosts semantics are closed by tests or explicit retirement.
- [ ] Three-mode SU authentication has secure migration, prompt, cancellation, replay, and fallback tests, or an evidence-backed retirement per mode.
- [ ] Full MagiskSU prompt/allow/deny/revoke/timeout/shared-UID/multiuser/namespace behavior passes
  with manager running, restarted, hidden, and unavailable.
- [ ] Dynamic `/system/bin` MagiskSU/applet exposure has a keep/retire decision and namespace,
  Hide/SuList, module, ordinary-app, and uninstall visibility tests.
- [ ] Vivo `do_mount_check`, Nox `/sbin`/PATH, SELinux-disabled/Waydroid, and pre-init storage safety have exact dispositions and qualification evidence.
- [ ] Hidden-manager/package and daemon-socket randomization each have a threat-modelled keep/retire decision and upgrade/recovery tests where retained.
- [ ] Hidden-manager create, upgrade, launch, signer-failure recovery, and restore pass on every
  advertised target class.
- [ ] Published debug and release artifacts contain no debug ADB-shell auto-grant; any internal lab
  flavor is unmistakable, non-publishable, and isolated from production signing/update channels.
...
- [ ] Every PR8 `deferred-to-PR#` row has transitioned to `inherited-and-tested`,
  `ported-and-tested-in-PR#`, or `retired-with-evidence`; the ledger schema/checker rejects every
  other final state.
- [ ] Every retained behavior is tested on the selected implementation; every retirement records user impact, migration guidance, and evidence.
- [ ] PR16 leaves no required follow-up PR for a faithful production Kitsune release.
