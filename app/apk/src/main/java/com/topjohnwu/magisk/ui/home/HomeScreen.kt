...
fun HomeScreen(
    ...
    if (activity != null) {
        val openInstall by activity.showInstall.collectAsStateWithLifecycle()
        LaunchedEffect(openInstall) {
            if (openInstall) {
                showInstallDialog = true
                activity.showInstall.value = false
            }
        }
    }
...
private fun UninstallComposableDialog(
    ...
    MagiskDialog(
        ...
        dismissText = stringResource(if (Info.isSystemMode) android.R.string.cancel else CoreR.string.restore_img),
        onDismiss = if (Info.isSystemMode) onDismiss else onRestoreImage,
    ) {
        Text(text = stringResource(if (Info.isSystemMode) CoreR.string.system_mode_uninstall_msg else CoreR.string.uninstall_magisk_msg))
    }
}
...
private fun EnvFixComposableDialog(
    ...
    val needsFullFix = Info.isSystemMode || code == 2 ||
