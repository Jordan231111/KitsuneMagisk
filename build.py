...
def execv(cmds: list, env=None):
    ...
    proc = subprocess.run(cmds, stdout=out, env=env, shell=is_windows)
    if cmds[0] == "cargo" and proc.returncode:
        raise SystemExit(128 - proc.returncode if proc.returncode < 0 else proc.returncode)
    return proc
...
def llvm_tool(name: str) -> Path:
    llvm_prebuilt = paths().ndk / "toolchains" / "llvm" / "prebuilt"
...
def build_apk(module: str, output_name=None):
    ...
    apk = f"{module_paths[-1]}-{build_type.lower()}.apk"
    source = Path("app", *module_paths, "build", "outputs", "apk", build_type.lower(), apk)
...
def load_config():
    ...
    # Default values
    config["version"] = f"31.0-kitsune-next.{commit_hash}"
