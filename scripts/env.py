...
def ensure_toolchain():
    ...
    llvm_hosts = list((paths().ndk / "toolchains/llvm/prebuilt").iterdir())
    if len(llvm_hosts) != 1:
        error("Expected one ONDK LLVM host toolchain")
    llvm_bin = llvm_hosts[0] / "bin"
    os.environ["PATH"] = f"{llvm_bin}{os.pathsep}{os.environ['PATH']}"
    for triple, target in {
        "thumbv7neon-linux-androideabi": "armv7a-linux-androideabi23",
        ...
        "riscv64-linux-android": "riscv64-linux-android35",
    }.items():
        normalized = triple.replace("-", "_")
        os.environ[f"CXX_{normalized}"] = str(llvm_bin / f"{target}-clang++")
        os.environ[f"AR_{normalized}"] = str(llvm_bin / f"llvm-ar{EXE_EXT}")
